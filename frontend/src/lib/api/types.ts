...
export type ResolutionState = 'pending' | 'running' | 'done' | 'failed';
export type ResolutionOutcome = 'confirmed' | 'enriched' | 'disputed' | 'ambiguous' | 'unmatched';
...
export interface FieldProtectionRequest {
...
export type TaskType = 'import_file' | 'import_directory' | 'resolve_book' | 'scan_isbn';
...
export interface MergeRequest {
  primary_id: string;
  secondary_id: string;
  prefer_metadata_from?: 'primary' | 'secondary' | 'higher_ingest_quality';
}
...
export interface RefreshAllMetadataResponse {
  count: number;
  task_ids: string[];
}
