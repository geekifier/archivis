<script lang="ts">
  ...
  const canSubmit = $derived(hasChanges && username.trim().length > 0 && !submitting);
...
<Dialog.Root {open} onOpenChange={handleOpenChange}>
  <Dialog.Content class="sm:max-w-md">
    ...
    <div class="space-y-4">
      <div>
        <Label for="edit-username" class="mb-1.5 block text-sm font-medium">Username</Label>
        <Input id="edit-username" type="text" bind:value={username} disabled={submitting} />
      </div>

      <div>
        <Label for="edit-email" class="mb-1.5 block text-sm font-medium">Email</Label>
        <Input
          id="edit-email"
          type="email"
