<script lang="ts">
  ...
  import type { FsDetectionResponse, WatchedDirectoryResponse, WatchMode } from '$lib/api/types.js';
  ...
  const showNativeWarning = $derived(
    watchMode === 'native' && detection !== null && detection.native_likely_works !== 'likely'
  );
  ...
  async function loadDeleteSourceSetting() {
    ...
    try {
      ...
      const entry = response.settings.find((s) => s.key === 'watcher.delete_source_after_import');
      if (entry) {
...
<Dialog.Root {open} onOpenChange={handleOpenChange}>
  <Dialog.Content class="sm:max-w-lg">
    ...
    <div class="space-y-4">
      ...
      {#if !isEditing}
        <div>
          <label for="watch-path" class="mb-1.5 block text-sm font-medium"> Directory Path </label>
          <div class="flex gap-2">
            ...
            <Button variant="outline" size="sm" onclick={() => (pickerOpen = true)}>Browse</Button>
          </div>
          {#if pathError}
            <p class="mt-1 text-xs text-destructive">{pathError}</p>
          {:else if pathInput.trim() && pathInput.trim() !== watchPath}
            <p class="text-center mt-2 text-sm text-muted-foreground ml-40">Press ⏎ to confirm</p>
          {/if}
        </div>
      {:else}
        <div>
      ...
      {#if !isEditing}
        <div class="flex items-center justify-between gap-4">
          <div class="space-y-0.5">
            <Label for="import-existing" class="text-sm font-medium">Import existing files</Label>
            <p class="text-xs text-muted-foreground">
              Scan and import all ebook files already present in this directory when it is first
              added.
            </p>
          </div>
          <Switch
      ...
      <div class="flex items-center justify-between gap-4">
        <div class="space-y-0.5">
          <Label for="delete-source" class="text-sm font-medium">Delete source after import</Label>
          <p class="text-xs text-muted-foreground">
            Remove the original file from this directory after it has been successfully imported
            into the library.
          </p>
        </div>
        <Switch
      ...
      {#if detection}
        <div class="space-y-2 rounded-lg border border-border bg-muted/50 px-4 py-3">
          ...
          <p class="text-xs text-muted-foreground/70">
            ...
            Kubernetes), the detection may not reflect the actual backing storage. When in doubt,
            use polling.
          </p>
        </div>
      {/if}
      ...
      {#if watchPath || isEditing}
          ...
          <div class="space-y-2">
            ...
            >
              ...
              <div class="flex-1">
                ...
                <p class="mt-0.5 text-xs text-muted-foreground">
                  Periodically scans for changes. Works everywhere — local disks, network shares,
                  container volumes, cloud storage mounts. Changes are detected within the polling
                  interval.
                </p>
              </div>
            </label>
            ...
            >
              ...
              <div class="flex-1">
                ...
                <p class="mt-0.5 text-xs text-muted-foreground">
                  Uses OS-level filesystem notifications (inotify on Linux, FSEvents on macOS) for
                  near-instant detection. Only works on local filesystems. Does NOT detect changes
                  made by other machines on network shares (NFS, SMB).
                </p>
              </div>
            </label>
          </div>
        ...
        {#if showNativeWarning}
          ...
          >
            ...
            <span>
              Detection suggests this path may not support native events. If you experience missed
              imports, switch to polling.
            </span>
          </div>
        {/if}
        ...
        {#if watchMode === 'poll'}
          <div>
            ...
            <Input id="poll-interval" type="number" min={5} max={3600} bind:value={pollInterval} />
            <p class="mt-1 text-xs text-muted-foreground">
              How often to check for new files. Lower values detect changes faster but use more
              resources.
            </p>
          </div>
        {/if}
    ...
    <Dialog.Footer>
      ...
      <Button onclick={handleSubmit} disabled={adding || (!watchPath.trim() && !isEditing)}>
        {#if adding}
