...
{#if open}
  ...
  >
    ...
    <div class="flex-1 overflow-y-auto">
      {#if loadingBookmarks}
        <div class="px-4 py-8 text-center text-sm text-muted-foreground">Loading bookmarks...</div>
      {:else if bookmarks.length === 0}
        <div class="px-4 py-8 text-center text-sm text-muted-foreground">No bookmarks yet</div>
      {:else}
        {#each bookmarks as bookmark (bookmark.id)}
          ...
          >
            ...
            <button class="min-w-0 flex-1 text-left" onclick={() => handleNavigate(bookmark)}>
