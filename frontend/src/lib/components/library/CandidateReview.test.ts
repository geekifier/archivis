...
describe('CandidateReview', () => {
	...
	beforeEach(() => {
		...
		book = createBookDetail({
			id: 'book-1',
			title: 'Test Book',
			authors: [{ id: 'a1', name: 'Original Author', sort_name: 'Author, Original', role: 'author', position: 0 }]
		});
	});
	...
	it('Apply calls api.identify.applyCandidate and onapply callback', async () => {
		const updatedBook = createBookDetail({ title: 'Updated Book' });
		mockApi.identify.applyCandidate.mockResolvedValue(updatedBook);
		...
		await vi.waitFor(() => {
			expect(mockApi.identify.applyCandidate).toHaveBeenCalledWith(
				'book-1',
				'c1',
				undefined
			);
	...
	it('Reject calls api.identify.rejectCandidate and onreject callback', async () => {
		mockApi.identify.rejectCandidate.mockResolvedValue(undefined);
		...
		await vi.waitFor(() => {
			expect(mockApi.identify.rejectCandidate).toHaveBeenCalledWith('book-1', 'c1');
		});
		await vi.waitFor(() => {
	...
	it('Undo calls api.identify.undoCandidate and onundo callback', async () => {
		const restoredBook = createBookDetail({ title: 'Restored Book' });
		mockApi.identify.undoCandidate.mockResolvedValue(restoredBook);
		...
		await vi.waitFor(() => {
			expect(mockApi.identify.undoCandidate).toHaveBeenCalledWith('book-1', 'c3');
		});
		await vi.waitFor(() => {
	...
	it('shows error message when API call fails', async () => {
		mockApi.identify.applyCandidate.mockRejectedValue(new Error('Network error'));
