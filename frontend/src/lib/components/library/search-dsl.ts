...
export const OPERATORS: OperatorDef[] = [
	...
	// Identifiers
	{ name: 'isbn', aliases: [], category: 'identifier', description: 'ISBN identifier' },
	{ name: 'asin', aliases: [], category: 'identifier', description: 'Amazon ASIN' },
	{
...
export function getOperatorSuggestions(prefix: string): SuggestionItem[] {
	...
	for (const op of OPERATORS) {
		...
		if (nameMatches || aliasMatch) {
			const aliasHint =
				op.aliases.length > 0 ? ` (alias: ${op.aliases.join(', ')})` : '';
			results.push({
				id: `op:${op.name}`,
				label: `${op.name}:`,
