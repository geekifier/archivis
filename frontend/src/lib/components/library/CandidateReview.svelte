...
<div class="space-y-4">
  ...
  {:else}
    ...
    {#each pendingCandidates as candidate (candidate.id)}
      <div
        class="rounded-lg border border-border bg-card shadow-sm {hasAppliedCandidate
          ? 'opacity-50'
          : ''}"
      >
        <!-- Candidate header -->
        ...
        <div class="px-4 py-3">
              ...
              <tbody class="divide-y divide-border/50">
                ...
                {#if candidate.title != null}
                  <tr class={!isFieldIncluded(candidate.id, 'title') ? 'opacity-40' : ''}>
                    ...
                    <td
                      class="py-1.5 text-xs {hasChange(candidate.title, book.title)
                ...
                {#if candidate.authors.length > 0}
                  <tr class={!isFieldIncluded(candidate.id, 'authors') ? 'opacity-40' : ''}>
                    ...
                    <td
                      class="py-1.5 text-xs {candidate.authors.join(', ') !==
                      book.authors.map((a) => a.name).join(', ')
                        ? 'font-medium text-primary'
                        : ''}"
                    >
                ...
                {#if candidate.publisher || book.publisher_name}
                  <tr>
                    ...
                    <td
                      class="py-1.5 text-xs {hasChange(candidate.publisher, book.publisher_name)
                        ? 'font-medium text-primary'
                        : ''}"
                    >
                      {candidate.publisher ?? '--'}
                ...
                {#if candidate.publication_date || book.publication_date}
                  <tr
                    class={candidate.publication_date != null &&
                    !isFieldIncluded(candidate.id, 'publication_date')
                      ? 'opacity-40'
                      : ''}
                  >
                    ...
                    <td
                      class="py-1.5 text-xs {hasChange(
                ...
                {#if candidate.series}
                  <tr class={!isFieldIncluded(candidate.id, 'series') ? 'opacity-40' : ''}>
                    ...
                    <td class="py-1.5 text-xs font-medium text-primary">
                      {candidate.series.name}{candidate.series.position != null
                ...
                {#if candidate.description}
                  <tr class={!isFieldIncluded(candidate.id, 'description') ? 'opacity-40' : ''}>
                    ...
                    <td
                      class="py-1.5 text-xs {hasChange(candidate.description, book.description)
                        ? 'font-medium text-primary'
                        : ''}"
                    >
          ...
          {#if candidate.cover_url}
            <div
              class="mt-3 flex items-start gap-3 border-t border-border pt-3 {!isFieldIncluded(
                candidate.id,
                'cover'
              )
                ? 'opacity-40'
