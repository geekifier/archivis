...
<Dialog.Root {open} onOpenChange={handleOpenChange}>
  <Dialog.Content class="flex max-h-[80vh] flex-col sm:max-w-lg">
    ...
    <div class="flex gap-2">
      ...
      <Button variant="outline" size="sm" onclick={handleManualGo} disabled={loading}>Go</Button>
    </div>
    ...
    <div class="min-h-[200px] flex-1 overflow-y-auto rounded-md border border-border">
      ...
      {:else}
        ...
        {#each entries as entry (entry.name)}
          <button
            class="flex w-full items-center gap-2 px-3 py-2 text-sm hover:bg-muted {!entry.is_dir &&
            mode === 'directory'
              ? 'opacity-50'
              : ''}"
    ...
    <Dialog.Footer class="flex justify-between gap-2">
      ...
      {#if mode === 'directory'}
        <Button onclick={handleSelect} disabled={loading || !!error}>Select This Directory</Button>
      {/if}
    </Dialog.Footer>
  </Dialog.Content>
</Dialog.Root>
