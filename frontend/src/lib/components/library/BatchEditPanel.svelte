<script lang="ts">
	...
	import type { MetadataStatus, TagEntry } from '$lib/api/index.js';
	...
	// --- Field values ---
	let language = $state('');
	let metadataStatus = $state<MetadataStatus>('identified');
	let rating = $state('');
	...
	const hasChanges = $derived(
		includeLanguage || includeStatus || includeRating || includePublisher || includeTags
	);
	...
	async function handleApply() {
		...
		try {
			...
			if (includeLanguage || includeStatus || includeRating || includePublisher) {
				const updates: Record<string, unknown> = {};
				if (includeLanguage) updates.language = language || undefined;
				if (includeStatus) updates.metadata_status = metadataStatus;
				if (includeRating) {
...
<Dialog.Root bind:open>
	<Dialog.Content class="max-w-lg">
		...
		<div class="max-h-[60vh] space-y-4 overflow-y-auto py-2">
			...
			<div class="space-y-1.5">
				<div class="flex items-center gap-2">
					<button
						type="button"
						class="flex size-4 items-center justify-center rounded border transition-colors {includeLanguage ? 'border-primary bg-primary' : 'border-muted-foreground/50'}"
			...
			<div class="space-y-1.5">
				<div class="flex items-center gap-2">
					<button
						type="button"
						class="flex size-4 items-center justify-center rounded border transition-colors {includeStatus ? 'border-primary bg-primary' : 'border-muted-foreground/50'}"
						onclick={() => (includeStatus = !includeStatus)}
						aria-label="Include status in update"
					>
						{#if includeStatus}
							<svg class="size-3 text-primary-foreground" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round">
								<polyline points="20 6 9 17 4 12" />
			...
			<div class="space-y-1.5">
				<div class="flex items-center gap-2">
					<button
						type="button"
						class="flex size-4 items-center justify-center rounded border transition-colors {includeRating ? 'border-primary bg-primary' : 'border-muted-foreground/50'}"
			...
			<div class="space-y-1.5">
				<div class="flex items-center gap-2">
					<button
						type="button"
						class="flex size-4 items-center justify-center rounded border transition-colors {includePublisher ? 'border-primary bg-primary' : 'border-muted-foreground/50'}"
				...
				{#if includePublisher}
					...
					{:else}
						...
						/>
						<p class="text-xs text-muted-foreground">Leave empty to clear publisher from all selected books.</p>
					{/if}
				{/if}
			</div>
			...
			<div class="space-y-1.5">
				<div class="flex items-center gap-2">
					<button
						type="button"
						class="flex size-4 items-center justify-center rounded border transition-colors {includeTags ? 'border-primary bg-primary' : 'border-muted-foreground/50'}"
				...
				{#if includeTags}
					...
					<div class="flex gap-1 rounded-md border border-input p-0.5">
						<button
							type="button"
							class="flex-1 rounded px-2 py-1 text-xs font-medium transition-colors {tagMode === 'add' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'}"
							onclick={() => (tagMode = 'add')}
						>
							Add tags
						</button>
						<button
							type="button"
							class="flex-1 rounded px-2 py-1 text-xs font-medium transition-colors {tagMode === 'replace' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'}"
		...
		{#if resultMessage}
			<div class="rounded-md border border-green-500/30 bg-green-500/10 px-3 py-2 text-sm text-green-700 dark:text-green-400">
				{resultMessage}
			</div>
		{/if}
		{#if applyError}
