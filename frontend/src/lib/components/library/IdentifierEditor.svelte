<script lang="ts">
	...
	const addDisabled = $derived(
		adding ||
			!addValue.trim() ||
			(addIsIsbn && (!addValidation || !addValidation.valid))
	);
	...
	const editSaveDisabled = $derived(
		saving ||
			!editValue.trim() ||
			(editIsIsbn && (!editValidation || !editValidation.valid))
	);
	...
	async function handleSaveEdit() {
		...
		try {
			const normalizedValue =
				editIsIsbn && editValidation
					? editValidation.normalized
					: editValue.trim();
					...
					{#each book.identifiers as ident (ident.id)}
						<tr class="border-b border-border/50">
							...
							{:else}
								...
								<td class="py-2 pr-3">
									<span
										class="inline-flex rounded-full px-2 py-0.5 text-xs font-medium {typeColorClass(ident.identifier_type)}"
									>
		...
		<div class="flex items-start gap-2">
			...
			<div class="flex-1 space-y-1">
				...
				{#if addIsIsbn && addValidation}
					...
					{#if addValidation.valid && addValidation.normalized !== addValue.replace(/[\s-]/g, '').toUpperCase()}
						<p class="text-xs text-muted-foreground">
