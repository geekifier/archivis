<script lang="ts">
	...
	import type { BookDetail, DuplicateLinkResponse } from '$lib/api/index.js';
	...
	let metadataPreference = $state<string>('primary');
	...
	const metadataPreferenceOptions = [
		...
		{ value: 'higher_confidence', label: 'Use higher confidence' }
	];
...
<Dialog.Root bind:open>
	<Dialog.Content class="max-w-5xl max-h-[90vh] overflow-y-auto">
		<Dialog.Header>
			...
			<Dialog.Description>
				Compare both books side by side. Choose which book to keep as the primary,
				then merge to combine all files and metadata.
			</Dialog.Description>
		</Dialog.Header>
		...
		{:else if bookA && bookB}
			<!-- Merge options -->
			<div class="mb-4 flex flex-wrap items-center gap-4 rounded-lg border border-border bg-muted/30 p-3">
			...
			<div class={diffClass(book.metadata_status, otherBook.metadata_status)}>
				...
				<dd class="flex items-center gap-2">
					...
					<span class="text-xs text-muted-foreground">
						({Math.round(book.metadata_confidence * 100)}% confidence)
					</span>
				</dd>
			</div>
