<script lang="ts">
	...
	const importedCount = $derived(
		progressData ? Number(progressData.imported ?? 0) : null
	);
	const skippedCount = $derived(
		progressData ? Number(progressData.skipped ?? 0) : null
	);
	const failedCount = $derived(
		progressData ? Number(progressData.failed ?? 0) : null
	);
	const processedCount = $derived(
		progressData ? Number(progressData.processed ?? 0) : null
	);
	const totalCount = $derived(
		progressData ? Number(progressData.total ?? 0) : null
	);
				...
				<div class="grid grid-cols-2 gap-2 text-xs">
					...
					{#if importedCount !== null}
						<div>
							...
							<span class="ml-1 font-semibold text-green-600 dark:text-green-400">{importedCount}</span>
						</div>
					{/if}
					{#if skippedCount !== null && skippedCount > 0}
						<div>
							<span class="text-muted-foreground">Skipped:</span>
							<span class="ml-1 font-semibold text-amber-600 dark:text-amber-400">{skippedCount}</span>
						</div>
					{/if}
				...
				<div class="rounded bg-muted/50 p-2 text-xs">
					...
					<div class="mt-1 flex flex-wrap gap-2">
						...
						{#if childrenSummary.cancelled > 0}
							<span class="text-amber-600 dark:text-amber-400">{childrenSummary.cancelled} cancelled</span>
						{/if}
					</div>
				</div>
			{/if}
