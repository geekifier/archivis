	...
	async function runAuthGuard() {
		...
		if (!auth.isAuthenticated && !isPublicPage) {
			const redirect =
				page.url.pathname === '/'
					? ''
					: `?redirect=${encodeURIComponent(page.url.pathname)}`;
			goto(`/login${redirect}`);
		}
	}
	...
	<div class="flex h-screen overflow-hidden">
		...
		>
			<div class="flex h-14 items-center border-b border-sidebar-border px-4">
				<a
					href="/"
					class="text-lg font-semibold text-sidebar-primary"
					title="Archivis Home"
				>
					<svg
						version="1.1"
			...
			<nav class="flex-1 space-y-1 overflow-y-auto p-3">
				...
				{#if isLibraryPage}
					...
					<div class="px-3 pb-1">
						<span
							class="text-xs font-semibold uppercase tracking-wider text-sidebar-foreground/60"
							>Format</span
						>
					</div>
