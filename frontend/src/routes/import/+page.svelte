...
<div class="space-y-6">
  ...
  {#if mode === 'upload'}
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Drag and drop files or click to browse. Supported formats: EPUB, PDF, MOBI, CBZ, FB2, TXT,
          DJVU, AZW3.
        </CardDescription>
      </CardHeader>
      <CardContent>
        ...
        {#if selectedFiles.length > 0}
          <div class="mt-4 space-y-2">
            <div class="flex items-center justify-between">
              ...
              <Button variant="ghost" size="sm" onclick={() => (selectedFiles = [])}>
                Clear all
              </Button>
            </div>
        ...
        <!-- ISBN scan note -->
        <div
          class="mt-4 flex items-start gap-2 rounded-md border border-border bg-muted/50 px-3 py-2.5"
        >
          <svg
          ...
          <div class="text-xs text-muted-foreground">
            <p>
              <strong class="font-medium text-foreground"
                >Scan book content for ISBNs after import</strong
              >
            </p>
            <p class="mt-0.5">
              Controlled by the <code class="rounded bg-muted px-1 py-0.5 text-xs"
                >scan_on_import</code
              >
              setting in the server's
              <code class="rounded bg-muted px-1 py-0.5 text-xs">[isbn_scan]</code> config section. When
              enabled, imported books are automatically scanned for ISBNs embedded in their content.
            </p>
          </div>
        </div>
  ...
  <Card>
    ...
    <CardContent>
      ...
      {:else if tasksError}
        <div
          class="flex items-center justify-center rounded-lg border border-dashed border-destructive/50 p-6"
        >
      ...
      {:else if sortedRecentTasks.length === 0}
        <div
          class="flex items-center justify-center rounded-lg border border-dashed border-border p-6"
        >
                ...
                {#if task.children_summary && task.children_summary.total > 0}
                  ...
                  >
                    <span class="inline-flex items-center gap-1">
                      {task.children_summary.total} subtask{task.children_summary.total === 1
                        ? ''
                        : 's'}
                      <svg
                        class="size-3 transition-transform {expandedTaskIds.has(task.id)
                          ? 'rotate-180'
                          : ''}"
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 24 24"
