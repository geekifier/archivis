<script lang="ts">
  ...
  const columnSortField: Record<string, 'sort_name' | 'name' | 'book_count'> = {
    name: 'sort_name',
    sort_name: 'sort_name',
    ...
    >
      <div class="text-center">
        {#if activeQuery}
          ...
          <p class="mt-1 text-sm text-muted-foreground">No authors match your search.</p>
          <Button
            variant="outline"
            class="mt-4"
            onclick={() => {
