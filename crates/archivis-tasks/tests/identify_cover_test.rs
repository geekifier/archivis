//! Integration tests for cover handling during candidate application.
...
async fn setup(
    ...
    IdentificationService<LocalStorage>,
    archivis_db::DbPool,
    LocalStorage,
) {
    ...
    let service = IdentificationService::new(pool.clone(), resolver, storage.clone(), data_dir);
